        ...
        for error_category in "${ERROR_CATEGORIES[@]}"; do
            for variant in "${VARIANTS[@]}"; do
                ...
                if [[ "$variant" == "MLP" ]]; then
                    ...
                    python train_ecr.py --error_category $error_category --task_name $TASK_NAME --split $split --variant $variant --backbone ${BACKBONE[0]} --ckpt_directory $CKPT_DIRECTORY_PATH --lr 0.000001
                fi
            done
        done
    done
